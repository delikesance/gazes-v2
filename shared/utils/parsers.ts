...
export function parseAnimePage(html: string): AnimeInfo {
    ...
    const genres = genresText.split(',').map((g: string) => g.trim()).filter(Boolean)
    ...
    const collectFromScripts = ($container: any, type: string) => {
        let scriptText = $container.find('script').map((_: number, s: any) => $(s).text()).get().join('\n')
        ...
        while ((m = re.exec(scriptText)) !== null) {
            ...
            // Skip placeholders
            if (!name || !url || name.toLowerCase() === 'nom' || url === '/catalogue/naruto/url') continue
            if (type === 'Manga') manga.push({ name, url })
            else seasons.push({ name, url, type })
        }
    }
...
export function parseAnimeResults(html: string): SearchResponse {
    ...
    const animeRegex = /<a[^>]*href="(https:\/\/anime-sama\.org\/catalogue\/[^\"]*)"[^>]*>[\s\S]*?<img[^>]*src="([^"]*)"[^>]*>[\s\S]*?<h3[^>]*>([^<]*)<\/h3>[\s\S]*?<p[^>]*>([^<]*)<\/p>[\s\S]*?<\/a>/g;
    ...
    while ((match = animeRegex.exec(html)) !== null) {
        ...
        console.log(`Parsed anime: title='${title.trim()}', id='${id}', url='${url}'`);
        results.push({
            title: title.trim(),
            id,
            image: image || '',
